<?php
    ...
    // 먼저 고유번호가 $login_user_num인 유저가 작성한 게시물들을 날짜가 빠른순으로 게시물 각각의 데이터를 배열에 담아 둔다.
    // $login_user_num가 작성한 집구경 게시물 등록날짜 기준 내림차순으로 데이터 조회(가장 최근에 업로드한 게시물이 먼저오도록(맨 위로 오도록)조회.)
    // $page 행 부터 $limit만큼 데이터 조회해서 가져오기.
    $sql = "select * from house_tour_post where user_num = $login_user_num order by post_regtime desc limit $limit offset $page";
	...
	while($post = $res->fetch_assoc()) { 
        ...
        $sql_user2 = "select * from house_tour_post where user_num = $post[post_num]";
