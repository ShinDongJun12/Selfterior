       ...
       /** String -> JSONObject 변환 메서드 (클라이언트로부터 받은 jsonString을 SONObject로 파싱한다.) */    
        ...
        public void run(){ //쓰레드를 사용하기 위해서 run()메서드 재정의
            ...
            try{ 
	            ...
	            // json의 기본 반환형은 Long형 이다. 이것을 다시 Int형으로 변환행야 한다.
                    int message_num = ((Long)jsonObject.get("message_num")).intValue();
                    int room_num = ((Long)jsonObject.get("room_num")).intValue(); // 채팅방 고유번호. 
                    String room_num_str = Integer.toString(room_num);

                    int user_num = ((Long)jsonObject.get("user_num")).intValue();
                    String user_num_str = Integer.toString(user_num);

                    String message_content = (String) jsonObject.get("message_content"); // ※ 해당 키 값을 통해 얻어지는 valye 값이 어떤 데이터인지 모르기 때문에 명시해 주어야한다.
                    String regtime = (String) jsonObject.get("regtime");
                    String message_type = (String) jsonObject.get("message_type");
                    String user_profile_image = (String) jsonObject.get("user_profile_image");                   
                    ...
                    sendGroupMsg(room_num_str, jsonString); // 채팅방에 들어와있는 클라이언트에게만 채팅 메시지 보내기.
                    clientMap= globalMap.get(room_num_str); //현재그룹의 해쉬맵을 따로 저장.
                    clientMap.put(user_num_str, this); //현재 ChatServerRec인스턴스를 클라이언트맵에 저장.
                    ...
                    globalMap.put(room_num_str,group);
