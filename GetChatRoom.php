...
// ※ 중고거래 게시물 고유번호에 따라 조회 쿼리 달라짐. ※
    
    // 그냥 채팅 목록창에 들어온 경우.
    if($post_num == 0)
    {
        // 해당 유저가 판매자 또는 구매자로 들어가있는 모든 채팅방을 마지막 메시지 등록날짜가 최신인 순으로 조회한다.
        $sql = "SELECT * FROM chat_room WHERE sales_user_num = $user_num OR purchase_user_num = $user_num ORDER BY regtime DESC LIMIT $limit offset $page";
    }
    // 판매자가 자신의 판매글 채팅 목록창에 들어온 경우.
    else
    {
        // 게시물 고유번호가 존재하면 해당 게시물에 대한 판매자가 속해있는 채팅방을 마지막 채팅 메시지 등록날짜가 최신인 순으로 조회한다. (이 경우는 자신이 해당게시물의 판매자 이므로 판매자 고유번호만 조회.)
        $sql = "SELECT * FROM chat_room WHERE post_num = $post_num AND sales_user_num = $user_num ORDER BY regtime DESC LIMIT $limit offset $page";
    }
