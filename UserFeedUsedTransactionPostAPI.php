<?php
    ...
    $targetUserNum = $_GET["targetUserNum"]; // 피드유저 고유번호 (int)
    ...
    // ★ 중고거래 게시물 중에서 '거래완료'를 제외한 ★ 중고거래 게시물 등록날짜 기준 내림차순으로 데이터 조회(가장 최근에 업로드한 게시물이 먼저오도록(맨 위로 오도록)조회.)
    // $page 행 부터 $limit만큼 데이터 조회해서 가져오기.
    $sql = "SELECT * FROM used_transaction_post WHERE user_num = $targetUserNum AND transaction_status NOT IN('거래완료') order by post_regtime desc limit $limit offset $page";
