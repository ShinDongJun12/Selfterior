...
// 아무것도 북마크 없을 때 예외처리할 것!!!!!!!!!!!!
    ...
    $sql = "select post_num, post_like_check from post_like where user_num = $login_user_num and post_like_check = 2 order by post_like_regtime desc limit $limit offset $page";
	...
	while($post = $res->fetch_assoc()) { 
        ...
        // (※좋아요는 기획상 집구경 게시물에서만 이용할 수 있으므로 북마크 정보는 '집구경게시물'에 한해서 가져온다.※)
        $sql_user2 = "select bookmark_check from bookmark where user_num = $login_user_num and post_num = $post[post_num] and bookmark_category = '집구경게시물'";
        $res_user2 = mysqli_query($conn, $sql_user2); // 다른 형식의 SQL 구문, INSERT, UPDATE, DELETE, DROP 등에서 성공하면 TRUE를, 실패하면 FALSE를 반환합니다.
        $num_row = mysqli_num_rows($res_user2); 

        // 사용자가 해당 게시물에 대해 북마크 버튼을 한번이라도 누른적이 있다면.
        if($num_row > 0){
            $post_bookmark_info = array(); // post_info 배열 생성
            $post_bookmark_info = mysqli_fetch_assoc($res_user2); // bookmark_check값을 받아온다.
        }
        // 한번도 북마크 버튼을 누른적이 없으면.
        else{

            $post_bookmark_info = array(); // post_info 배열 생성
            $post_bookmark_info['bookmark_check'] = 0; // bookmark_check값에 0값을 넣는다.
        }
        ...
        $data = [
            ...
            'bookmark_check' => $post_bookmark_info['bookmark_check'], // 게시물 북마크 체크여부 (한번도 북마크 버튼을 누른적이 없다면 null값을 전달한다)
            'post_like_check' => $post['post_like_check'], // 게시물 좋아요 체크여부 (한번도 좋아요 버튼을 누른적이 없다면 null값을 전달한다)
