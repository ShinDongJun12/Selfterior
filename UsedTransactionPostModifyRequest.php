<?php
    ...
    $postNumInt = (int)$postNum; // (String) -> (int)

    $itemPriceStr = $_POST["itemPriceStr"]; // 아이템 가격 (String)
    $itemPrice = (int)$itemPriceStr; // (String) -> (int)

    $postTitle = $_POST["postTitle"]; // 게시물 제목
    $itemCategory = $_POST["itemCategory"]; // 아이템 카테고리
    $saleAddress = $_POST["saleAddress"]; // 판매자 주소
    ...
    $sql = "SELECT * FROM used_transaction_post WHERE post_num = $postNumInt"; 
    ...
    if($exist > 0){
        ...
        // 게시물 이미지들이 담겨있는 폴더명
        $postImageDirName = $postNum.'_used_transaction_post_images';
    
        $delete_path = "usedTransactionPostImages/$postImageDirName";
        ...
        $postDirName = $postNum.'_'.$dirBaseName; // ex) 23_used_transaction_post_images
        ...
        $sql1 = "UPDATE used_transaction_post SET post_title ='$postTitle', item_category ='$itemCategory', item_price ='$itemPrice', sale_address ='$saleAddress', post_content ='$postContent', post_imgPath = '$urlList' WHERE post_num = $postNum";
        ...
        if($res1) {
            ...
            // 중고거래 게시물 변경된 내용을 조회해서 클라이언트에 넘겨주고 변경된 내용을 바로 적용시켜준다.
            ...
            while($post_info = $res->fetch_assoc()) { 
                ...
                $data = [
                    ...
                    // 중고거래 게시물에서 가져올 수 있는 데이터만 보내는 듯.
                    ...
                    'post_imgPath' => $post_info['post_imgPath'], // 게시물 이미지 URL(저장된 이미지 경로 전체)
                    'post_regtime' => $post_info['post_regtime'], // 게시물 등록날짜
                    'item_category' => $post_info['item_category'], // 아이템 카테고리
                    'item_price' => $post_info['item_price'], // 아이템 가격
                    'sale_address' => $post_info['sale_address'], // 판매자 주소
                    'post_view_count' => $post_info['post_view_count'], // 게시물 조회수
                    'transaction_status' => $post_info['transaction_status'], // 거래상태
