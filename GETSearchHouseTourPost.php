<?php
    ...
    // 집구경 게시물 제목 or 내용에 검색 키워드가 포함되어 있는 집구경 게시물의 데이터를 조회한다.
    // 집구경 게시물 등록날짜 기준 내림차순으로 데이터 조회(가장 최근에 업로드한 게시물이 먼저오도록(맨 위로 오도록)조회.)
    // $page 행 부터 $limit만큼 데이터 조회해서 가져오기.
    $sql = "SELECT * FROM house_tour_post WHERE post_title LIKE '%$search_keyword%' OR post_content LIKE '%$search_keyword%' order by post_regtime desc limit $limit offset $page";
