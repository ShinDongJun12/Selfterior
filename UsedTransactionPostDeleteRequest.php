<?php 
    ...
    $resultNum = (int)$postNum;
        ...
        if($exist > 0){
            ...
            $sql = "DELETE FROM used_transaction_post WHERE post_num = $resultNum"; 
            $ret = mysqli_query($conn, $sql); 

            // 북마크 카테고리가 '중고거래게시물'이고 해당 게시물 고유번호로 등록된 북마크 데이터를 모두 삭제한다.
            $sql2 = "DELETE FROM bookmark WHERE post_num = $resultNum AND bookmark_category = '중고거래게시물'"; 
            $ret2 = mysqli_query($conn, $sql2); 

// ※ 해당 게시물에 대한 채팅방 목록들 어떻게 할 건지 고민해야함 ※
   
            // 해당 고유번호를 가진 게시물에 대한 게시물 데이터,북마크 등 모든 데이터 삭제 성공.
            if($ret && $ret2){
