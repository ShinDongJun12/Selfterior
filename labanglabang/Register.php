<?php 
    ...
    $userUniqueIdentifier = null; // 유저 식별자 고번호 값(소셜 로그인으로 가입시 존재하지만 이메일 회원가입시 null값으로 생성한다.)

    // 비밀번호 암호화 처리
    $userPassword = md5($userPassword);

    $result=mysqli_query($conn, "INSERT INTO members(user_email, user_pass, user_token, user_nickname, platform_type, user_regtime, profile_image, recent_broadcast_date, user_introduction, user_unique_identifier) VALUES ('$userEmail','$userPassword','$userToken','$userNickname','$userPlatformType',now(),'$userProfileImg',now(),'$userIntroduction','$userUniqueIdentifier')");
    // mysqli_close($conn);
