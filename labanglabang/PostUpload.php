<?php
    ...
    // $basename = basename( $_FILES['uploaded_file']['name']); // $_FILES['uploaded_file']['name'] -> 실제 파일명.
    // echo "베이스네임 : $basename<br>"; 

    // 클라이언트로 보낼 응답 배열
    $result = array();

    // post_img_path값을 ''으로 하고 나머지 정보들은 그대로 DB에 저장한다. (post_num가 생긴다.).
    $sql1 = "INSERT INTO post (user_num, post_category, post_title, post_content, post_img_path, post_regtime)";
    $sql1.= " VALUES ($userNum, '$postCategory', '$postTitle', '$postContent', 'mmm', now())";
    ...
    if($res1) {
        ...
        $postUploadDir = "postImages"; // 게시물 이미지 저장할 폴더명
        ...
        if (!is_dir($postDirPath)) {
            ...
            if($cntImage > 0) {
                ...
                for($i=0; $i<$cntImage; $i++) { 
                    ...
                    $oldName = $_FILES['image'.$i]["name"]; //ex) example.jpg (Image1.jpg)
                    ...
                    $path = "$postUploadDir/$uploadDir/$name"; // 서버에 이미지를 저장할 경로. ex) postImages/23_post_images/게시물고유번호_1.jpg
            ...
            $sql2 = "UPDATE post SET post_img_path = '$uriList', post_regtime = now() WHERE post_num = $get_post_num";
            ...
            if($res2) {
                ...
                // 게시물 등록날짜 기준 내림차순으로 데이터 조회(가장 최근에 업로드한 게시물이 먼저오도록(맨 위로 오도록)조회.)
                // $page 행 부터 $limit만큼 데이터 조회해서 가져오기.
                $sql = "SELECT * FROM post WHERE post_num = $get_post_num";
        ...
        // 폴더가 존재하면 
            ...
            // DB에 게시물의 post_img_path, post_regtime 값을 추가적으로 저장한다.
            $sql3 = "UPDATE post SET post_img_path = '$uriList', post_regtime = now() WHERE post_num = $get_post_num";
            ...
            if($res3) {
                ...
                // while($post = $res->fetch_assoc()) { 
                    ...
                    // ***** 게시물 데이터 최종 합치기 *****
                    // 게시물 데이터 + 게시물 작성자 정보(닉네임, 프로필 이미지)를 $data에 다시 저장.
                    $data = [
